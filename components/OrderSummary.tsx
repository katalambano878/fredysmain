    ...
    <div className="bg-white rounded-xl shadow-sm p-6 sticky top-4">
        ...
        {items.map((item) => (
          <div key={`${item.id}-${item.variant || 'novar'}`} className="flex space-x-4">
            ...
            <div className="flex-1">
              ...
              <p className="text-gray-900 font-bold mt-1">{moneyLabel(item.price)}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="border-t border-gray-200 pt-4 space-y-3">
        <div className="flex justify-between text-gray-700">
          <span>Subtotal</span>
          <span className="font-semibold">{moneyLabel(subtotal)}</span>
        </div>
        <div className="flex justify-between text-gray-700">
          <span>Shipping</span>
          <span className="font-semibold">
            {shipping === 0 ? 'FREE' : moneyLabel(shipping)}
          </span>
        </div>
      ...
      <div className="border-t border-gray-200 mt-4 pt-4">
        <div className="flex justify-between items-center">
          ...
          <span className="text-2xl font-bold text-gray-900">{moneyLabel(total)}</span>
        </div>
      </div>
